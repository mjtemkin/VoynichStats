...
def pairwise_asymmetry(lines, classes, suffix_len=2, min_total=10):
...
for label, lines in [('A', lines_A), ('B', lines_B)]:
    ...
    real_asyms = pairwise_asymmetry(lines, classes)
    real_avg = sum(real_asyms)/len(real_asyms)
    print(f"Real average |asymmetry| ({len(real_asyms)} pairs): {real_avg:.4f}")

    # Run MANY shuffle trials to build a proper null distribution of the
    # average-asymmetry statistic, not just one shuffle
    n_trials = 200
    shuf_avgs = []
    for t in range(n_trials):
        shuf = shuffle_lines(lines, seed=t)
        shuf_asyms = pairwise_asymmetry(shuf, classes)
        if shuf_asyms:
            shuf_avgs.append(sum(shuf_asyms)/len(shuf_asyms))

    shuf_mean = sum(shuf_avgs)/len(shuf_avgs)
    shuf_var = sum((x-shuf_mean)**2 for x in shuf_avgs)/len(shuf_avgs)
    shuf_sd = math.sqrt(shuf_var)

    z = (real_avg - shuf_mean)/shuf_sd if shuf_sd > 0 else float('inf')
    # empirical p-value: fraction of shuffle trials with avg asymmetry >= real
    p_empirical = sum(1 for x in shuf_avgs if x >= real_avg) / len(shuf_avgs)

    print(f"Null distribution from {n_trials} shuffles: mean={shuf_mean:.4f}, sd={shuf_sd:.4f}")
    print(f"Z-score: {z:.2f}")
    print(f"Empirical p-value (fraction of shuffles >= real): {p_empirical:.4f}")
